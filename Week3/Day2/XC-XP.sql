...
SELECT firstname, surname  FROM customers WHERE ID BETWEEN 1 AND 3 ORDER BY firstname ASC
-- All last names (no other columns!), in reverse alphabetical order (Z-A)
SELECT surname  FROM customers ORDER BY surname ASC
...
SELECT first_name || ' ' || last_name AS full_name, email FROM customer ORDER BY first_name DESC
...
SELECT film_id, title, description, length, rental_rate FROM film WHERE title = 'Apocalypse now'

-- No luck finding your movie? Maybe you made a mistake spelling the name. Write a query to get 
-- the film ID, title, description, length and the rental rate of all the movies 
-- starting with the two first letters of your favorite movie.
SELECT film_id, title, description, length, rental_rate FROM film WHERE title LIKE 'Ap%'
...
?????SELECT * FROM film ORDER BY rental_rate WHERE index BETWEEN 10 AND 20
SELECT * FROM film

???SELECT title, rental_rate, row_number() over(ORDER BY rental_rate) num  FROM film WHERE num BETWEEN 10 AND 20
...
-- Bonus You want to be able to see how your sellers have been doing? Write a query 
-- to get the customer’s id, names (first and last), the amount and the date of payment ordered by the 
-- id of the staff member who sold them the dvd.
SELECT customer.customer_id, customer.first_name || ' ' || customer.last_name AS full_name, 
payment.payment_date, payment.amount, payment.staff_id
FROM payment 
INNER JOIN customer
